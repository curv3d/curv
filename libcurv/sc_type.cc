...
SC_Type SC_Type::Bool(unsigned n) {
    if (n == 1)
        return {Type::Bool()};
    assert(n >= 2 && n <= 4);
    return {makev<Array_Type>(n, Type::Bool())};
}
SC_Type SC_Type::Bool32(unsigned n) {
    if (n == 1)
        return {Type::Bool32()};
    assert(n >= 2 && n <= 4);
    return {makev<Array_Type>(n, Type::Bool32())};
}
SC_Type SC_Type::Num(unsigned n) {
    if (n == 1)
        return {Type::Num()};
    assert(n >= 2 && n <= 4);
    return {makev<Array_Type>(n, Type::Num())};
}
SC_Type SC_Type::Vec(SC_Type base, unsigned n) {
  #if !defined(NDEBUG)
    auto plex = base->plex_type_;
    assert(plex == Plex_Type::Bool ||
           plex == Plex_Type::Num ||
           plex == Plex_Type::Bool32);
    assert(n >= 1 && n <= 4);
  #endif
    return {makev<Array_Type>(n, CType{base})};
}
SC_Type SC_Type::Mat(int n) {
    assert(n >= 2 && n <= 4);
    return {makev<Array_Type>(n, CType{makev<Array_Type>(n, Type::Num())})};
}

SC_Type
SC_Type::Array(SC_Type etype, unsigned n)
{
    return {make<Array_Type>(n, CType{etype})};
}
